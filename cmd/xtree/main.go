...
// legacyTokenCache is the per-tool cache xtree kept before the family shared
...
func run() int {
	...
	usage := func(w io.Writer) {
		...
		fmt.Fprintln(w, "       xtree auth [--json]")
		fmt.Fprintln(w)
		...
		fmt.Fprintln(w, "`xtree auth` reports that session without starting a sign-in.")
		fmt.Fprintln(w)
	...
	if len(args) > 0 && args[0] == "auth" {
		return spauth.AuthCommand(context.Background(), "xtree", legacyTokenCache(), args[1:], os.Stdout, os.Stderr)
	}
