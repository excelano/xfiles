...
// Package xfiles exists for one reason: //go:embed cannot contain ".." or
// escape its own package directory, so a package under cmd/ cannot reach
// skills/ at the repo root. Moving the canonical files would break the
// GitHub-browsable path, and symlinking would break the raw-URL curl fallback,
// because raw.githubusercontent serves a symlink as the text of its target. A
// root-level package that all five cmd/ mains import is the way out — and it
// means the shared skill is written once, with no binary designated its owner.
//
...
// `xcp --install-skill` — true everywhere.
//
...
// skillTool is the name the version stamp reports the skill as documenting.
// Deliberately a constant rather than the invoked name: five binaries share
// this one skill, and if the stamp named whichever one ran, the bytes would
// differ per binary and the idempotence check would report an update every
// time they alternated.
const skillTool = "xfiles"

// skillCheckCmd is the command the stamp tells a reader to run. It is separate
// from skillTool only here, because "xfiles" is the family and the skill but
// not a binary — `xfiles -V` would be a dead instruction. Everywhere else in
// the fleet the two names coincide.
const skillCheckCmd = "xcp"

// skillName is the directory under ~/.claude/skills/, matching skills/<name>/
// in the repo so a manual `cp -r` needs no rename either.
const skillName = "xfiles"
...
func stampSkill(body, version string) string {
	note := fmt.Sprintf(
		...
		skillTool, version, skillCheckCmd, skillCheckCmd)
	const fence = "---\n"
